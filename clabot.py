    ...
    def do_POST(self):
        ...
        try:
            ...
            if content_type == "application/json":
                ...
                json_data = re.sub("^payload=", "", unquote(post_data.decode("utf-8")))
            else:
...
class PullRequestHandler(GithubHookHandler):
    ...
    def _check_cla(self, users, pull_request):
        ...
        if cache_cr:
            ...
            res = cache_cr.execute(SQL_SELECT_HIT_CACHE % placeholders, users)
        ...
        if cache_cr:
            if users_sign:
                ...
                res = cache_cr.execute(
                    SQL_SELECT_MISS_CACHE_USERS % placeholders, users_sign
                )
                if res:
                    users_sign_notify = res.fetchall()
                    cache_cr.execute(SQL_DELETE_MISS_CACHE % placeholders, users_sign)
            ...
            if users_no_sign or users_oca_no_sign:
                ...
                res = cache_cr.execute(SQL_SELECT_MISS_CACHE_PR, (pull_request,))
    ...
    def handle_payload(self, event):
        ...
        res = requests.get(
            base_url + path, params=params, auth=HTTPBasicAuth(login, password)
        )
        commits = res.json()
        ...
        users_no_login = self._cleanup_users_no_login(users_no_login, pull_request)
        ...
        if users_no_sign or users_oca_no_sign or users_no_login:
            ...
            if send_miss_notification:
                ...
                _logger.info(
                    "PR: %s/%s#%s: CLA notification already sent", owner, repo, number
                )
    ...
    def _cleanup_users_no_login(self, users_no_login, pull_request):
        ...
        res = cache_cr.execute(SQL_SELECT_MISS_CACHE_NOLOGIN_PR, (pull_request,))
    ...
    def _insert_miss_no_login(self, users_no_login, pull_request):
        ...
        if not cache_cr:
            return
        for _name, email in users_no_login:
            cache_cr.execute(SQL_INSERT_MISS_CACHE_NOLOGIN, (email, pull_request))
...
def run(config):
    ...
    if cache_file:
        ...
        try:
            ...
            _logger.error(
                'unable to create or use cache file "%s": %s', cache_file, err
            )
